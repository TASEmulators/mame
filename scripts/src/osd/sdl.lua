...
function maintargetosdoptions(_target,_subtarget)
	...
	elseif not _OPTIONS["MAIN_SHARED_LIB"] then
	...
	if BASE_TARGETOS=="unix" and _OPTIONS["targetos"]~="macosx" and _OPTIONS["targetos"]~="android" and _OPTIONS["targetos"]~="asmjs" and not _OPTIONS["MAIN_SHARED_LIB"] then
		links {
			"SDL2_ttf",
		}
	...
	if _OPTIONS["targetos"]=="macosx" then
		...
		if _OPTIONS["with-bundled-sdl2"]~=nil then
			if _OPTIONS["targetos"]~="android" then
				if _OPTIONS["targetos"]=="linux" then
					includedirs {
						MAME_DIR .. "3rdparty/SDL2-override/linux",
					}
					defines {
